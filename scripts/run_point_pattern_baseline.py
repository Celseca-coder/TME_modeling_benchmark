#!/usr/bin/env python
"""Point pattern baseline over all datasets, tasks and schemes.

Feature: per region, point process summary statistics (Ripley's K, Besag's L, pair correlation,
variogram) computed from cell coordinates, often evaluated at multiple radii. Features are
calculated within the tissue foreground.
"""
...
def run(dataset_names, seeds, data_root=None) -> pd.DataFrame:
    ...
    for name in dataset_names:
        ...
        for task in ds.task_ids:
            ...
            featurizer = lambda: PointPatternFeaturizer()
        ...
        for gt in ds.validation_config.get("generalization_tests", []):
            ...
            featurizer = lambda c=cell_type_col: PointPatternFeaturizer(cell_type_col=c)
...
def main():
    ...
    ap.add_argument("--output", default=str(_CODE / "results" / "point_pattern_benchmark.csv"))
