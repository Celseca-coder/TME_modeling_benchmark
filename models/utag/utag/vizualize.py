...
def add_spatial_image(
    ...
    adata.uns["spatial"]["image"]["scalefactors"] = {
...
def add_scale_box_to_fig(
    img: Array, ax, box_width: int = 100, box_height: float = 3, color: str = "white"
) -> Array:
    ...
    rect = patches.Rectangle(
        (x - box_width, y * (1 - box_height / 100)),
        box_width,
        y * (box_height / 100),
        linewidth=0.1,
        edgecolor="black",
...
def rgbfy_multiplexed_image(
    ...
    for i in range(3):
        if scale_method == "contrast_stretch":
            ...
            output_img[:, :, i] = adjust_gamma(
                output_img[:, :, i], gamma=gamma, gain=gain
            )
            # output_img[:,:,i] = rescale(output_img[:,:,i], contrast_percentile)
...
def adj2chord(adjacency_matrix: Array, size: int = 300):
    hv.output(fig="svg", size=size)

    links = (
        adjacency_matrix.stack()
        .reset_index()
        .rename(columns={"level_0": "source", "level_1": "target", 0: "value"})
        .dropna()
    )
    ...
    nodes = pd.DataFrame(
        order2ind.keys(), index=order2ind.values(), columns=["name"]
    ).reset_index()
