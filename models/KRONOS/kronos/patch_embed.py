...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if not self.flatten_embedding:
            assert self.flatten_embedding, "flatten_embedding=False not supported. Check the implementation of PatchEmbed."
            H, W = embed.size(2), embed.size(3)
            x_ = []
            for i in range(C):
                x_.append(x[:, i*H*W:(i+1)*H*W, :].reshape(-1, H, W, self.embed_dim)) # B H W C for each marker
            x = x_
        return x
