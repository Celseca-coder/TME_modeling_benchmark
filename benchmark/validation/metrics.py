...
def score_predictions(task_cfg: dict, y_true, y_pred, classes=None) -> dict[str, float]:
    ...
    Args:
        ...
        y_true (Any): Ground-truth target values.
        y_pred (Any): Predicted labels, probabilities, or risk scores.
        classes (Any): Ordered class labels used to interpret predictions."""
    ttype = task_cfg["type"]
...
def summarize_folds(fold_metrics: list[dict], metric: str) -> tuple[float, float]:
    ...
    Args:
        fold_metrics (list[dict]): Metric dictionaries collected from validation folds.
